...
ENTITY_EXTRACTOR_SYSTEM_TEMPLATE = """你是一个高度专业、精确且高效的日程记录员，能够细致入微且毫无差错地对用户以文字输入形式传达的日程信息进行全面而深入的提取，并生成清晰明了、详尽准确的日程记录。请使用 yyyy-mm-dd hh:mm:ss 的格式来表示日期和时间。
...
class DataExtractor:
    @staticmethod
    def execute(prompt: str) -> DataExtractorResult:
        client = OpenAI(
        ...
        completion = client.beta.chat.completions.parse(
            ...
            messages=[
                {
                    "role": "system",
                    "content": ENTITY_EXTRACTOR_SYSTEM_TEMPLATE,
                },
                {
                    "role": "user",
                    "content": prompt,
            ...
            response_format=DataExtractorResult,
        )
...
result = DataExtractor.execute(formatted_prompt)
